...
class base_complex_model(nn.Module):
    def __init__(self):
        ...
        #self.channelattention1 = ChannelAttention(channel=128, reduction=16)
        ...
        #self.channelattention2 = ChannelAttention(channel=128, reduction=16)
        ...
        #self.channelattention3 = ChannelAttention(channel=128, reduction=16)
        ...
        #self.channelattention4 = ChannelAttention(channel=128, reduction=16)
        ...
        #self.channelattention5 = ChannelAttention(channel=128, reduction=16)
        ...
        #self.channelattention6 = ChannelAttention(channel=128, reduction=16)
    ...
    def forward(self,x):
        ...
        x = self.batchnorm1(x)
        x = self.maxpool1(x)
        # ca = self.channelattention1(x)
        # x = self.conv2(x*ca)
        x = self.conv2(x)
        x = F.relu(x)
        x = self.batchnorm2(x)
        x = self.maxpool2(x)
        #ca = self.channelattention2(x)
        x = self.conv3(x)
        x = F.relu(x)
        x = self.batchnorm3(x)
        x = self.maxpool3(x)
        #ca = self.channelattention3(x)
        x = self.conv4(x)
        x = F.relu(x)
        x = self.batchnorm4(x)
        x = self.maxpool4(x)
        #ca = self.channelattention4(x)
        x = self.conv5(x)
        x = F.relu(x)
        x = self.batchnorm5(x)
        x = self.maxpool5(x)
...
def main():
    ...
    data = torch.rand(1,2,4800)
    output = model(data)
    print(model)
    #print(model.residual1.shape)
    print(output[1])
    print(F.softmax(output[1], dim=1))
    print(F.log_softmax(output[1], dim=1))
